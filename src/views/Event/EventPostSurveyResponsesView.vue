<template>
  <div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="flex items-center gap-3 mb-6">
      <router-link
        :to="{ name: EventRouteName.EVENT, params: { uuid: eventId } }"
...
import { EVENT_POST_SURVEY_RESPONSES } from "@/graphql/eventPostSurveys";
...
const responses = computed<ResponseItem[]>(
  () => result.value?.eventPostSurveyResponses ?? []
);
...
// Returns schema field definitions (without data filtering) for CSV header generation
...
function openModal(response: ResponseItem) {
  ...
  modalFields.value = extractFields(response.schema, response.data ?? {});
...
function exportCsv() {
  ...
  a.href = url;
  a.download = `survey-${props.surveyId}-responses.csv`;
  a.click();
