...
onMounted(() => {
  ...
  if (lastEventUpdate && (!lastCalendarRefresh || parseInt(lastEventUpdate) > parseInt(lastCalendarRefresh))) {
    console.log('Calendar Agenda: Detected recent event update, forcing refresh');
  ...
  return {
    ...
    events: async (
      ...
      if (forceRefresh.value) {
        ...
        console.log('Calendar Agenda: Fresh data fetched, events:', result?.searchEvents?.elements?.length);
      } else {
