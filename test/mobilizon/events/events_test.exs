defmodule Mobilizon.EventsTest do
  ...
  describe "validate_capacity_not_below_participant_count" do
    test "rejects update when setting maximum_attendee_capacity below current participant count" do
      actor = insert(:actor)
      event =
        insert(:event,
          organizer_actor: actor,
          options: %{maximum_attendee_capacity: 10}
        )

      # Add 3 participants (role :participant counts toward capacity)
      insert(:participant, event: event, actor: insert(:actor), role: :participant)
      insert(:participant, event: event, actor: insert(:actor), role: :participant)
      insert(:participant, event: event, actor: insert(:actor), role: :participant)

      assert Events.count_participant_participants(event.id) == 3
      ...
      attrs = %{
        ...
        options: %{maximum_attendee_capacity: 2}
      }

      assert {:error, :update, %Ecto.Changeset{} = changeset, _} =
               Events.update_event(event, attrs)

      assert %{options: ["Cannot set participant limit below current participant count"]} =
               errors_on(changeset)
    end

    test "allows update when setting maximum_attendee_capacity at or above current participant count" do
      actor = insert(:actor)
      event =
        insert(:event,
          organizer_actor: actor,
      ...
      assert Events.count_participant_participants(event.id) == 2
      ...
      attrs = %{
        ...
        begins_on: event.begins_on,
        organizer_actor_id: event.organizer_actor_id,
        options: %{maximum_attendee_capacity: 5}
      }

      assert {:ok, updated} = Events.update_event(event, attrs)
      assert updated.options.maximum_attendee_capacity == 5
    end

    test "allows update when already over capacity (e.g. changing waitlist_auto_promote)" do
      actor = insert(:actor)
      event =
        insert(:event,
          organizer_actor: actor,
      ...
      # Exceed capacity: 3 participants, limit 2
      insert(:participant, event: event, actor: insert(:actor), role: :participant)
      insert(:participant, event: event, actor: insert(:actor), role: :participant)
      insert(:participant, event: event, actor: insert(:actor), role: :participant)
      ...
      assert {:ok, updated} = Events.update_event(event, attrs)
      assert updated.options.waitlist_auto_promote == false
      assert updated.options.maximum_attendee_capacity == 2
    end
  end
