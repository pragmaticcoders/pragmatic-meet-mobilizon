    ...
    case Mobilizon.Actors.member?(actor_id, group_id) do
      ...
      false ->
        ...
        case Mobilizon.Actors.get_actor(group_id) do
          %Actor{type: :Group} = group ->
            ...
            case Mobilizon.Actors.create_member(%{
              ...
              {:ok, member} ->
                ...
                # If the group requires approval (not_approved role), notify group admins
                if role == :not_approved do
                  Logger.info("Group membership requires approval, admins will be notified")
                end
          ...
          _ ->
            Logger.warn("Group #{group_id} not found or is not a group")
            :ok
        end
    end
  end
