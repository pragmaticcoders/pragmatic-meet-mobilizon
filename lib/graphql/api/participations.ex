      ...
      false ->
        ...
        case Mobilizon.Actors.get_actor(group_id) do
          %Actor{type: :Group} = group ->
            if Member.can_be_joined(group) do
              ...
              case Mobilizon.Actors.create_member(%{
                ...
                {:ok, member} ->
                  Logger.info(
                    "Successfully auto-added actor #{actor_id} to group #{group_id} with role #{role}"
                  )
              ...
              Logger.info(
                "Group #{group_id} is invite-only, skipping auto-join for actor #{actor_id}"
              )
