defmodule Mobilizon.Events do
  ...
  @spec filter_pending_group_approval(Ecto.Queryable.t()) :: Ecto.Query.t()
  defp filter_pending_group_approval(query) do
  ...
  alias Mobilizon.Events.EventPostSurvey

  @spec list_event_post_surveys(integer | String.t()) :: [EventPostSurvey.t()]
  def list_event_post_surveys(event_id) do
    EventPostSurvey
  ...
  @spec get_event_post_survey(integer | String.t()) ::
          {:ok, EventPostSurvey.t()} | {:error, :not_found}
  def get_event_post_survey(id) do
    case Repo.get(EventPostSurvey, id) do
      %EventPostSurvey{} = survey -> {:ok, survey}
      nil -> {:error, :not_found}
    end
  end

  @spec create_event_post_survey(integer | String.t(), map()) ::
          {:ok, EventPostSurvey.t()} | {:error, Ecto.Changeset.t()}
  def create_event_post_survey(event_id, attrs) do
    attrs_with_event = Map.put(attrs, :event_id, event_id)

    %EventPostSurvey{}
    |> EventPostSurvey.changeset(attrs_with_event)
    |> Repo.insert()
  end

  @spec update_event_post_survey(EventPostSurvey.t(), map()) ::
          {:ok, EventPostSurvey.t()} | {:error, Ecto.Changeset.t() | :not_draft}
  def update_event_post_survey(%EventPostSurvey{status: "draft"} = survey, attrs) do
    survey
    |> EventPostSurvey.update_changeset(attrs)
    |> Repo.update()
  end

  def update_event_post_survey(%EventPostSurvey{}, _attrs), do: {:error, :not_draft}

  @spec update_event_post_survey_status(EventPostSurvey.t(), String.t()) ::
          {:ok, EventPostSurvey.t()} | {:error, Ecto.Changeset.t()}
  def update_event_post_survey_status(%EventPostSurvey{} = survey, status) do
    survey
    |> EventPostSurvey.status_changeset(status)
    |> Repo.update()
  end
end
