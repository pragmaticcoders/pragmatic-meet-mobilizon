defmodule Mobilizon.Actors do
  ...
  alias Mobilizon.Actors.GroupPostSurvey

  @spec list_group_post_surveys(integer | String.t()) :: [GroupPostSurvey.t()]
  def list_group_post_surveys(group_id) do
    GroupPostSurvey
    |> Ecto.Query.where([s], s.group_id == ^group_id)
    |> Ecto.Query.order_by([s], asc: s.inserted_at)
    |> Repo.all()
  end

  @spec get_group_post_survey(integer | String.t()) ::
          {:ok, GroupPostSurvey.t()} | {:error, :not_found}
  def get_group_post_survey(id) do
    case Repo.get(GroupPostSurvey, id) do
      %GroupPostSurvey{} = survey -> {:ok, survey}
      nil -> {:error, :not_found}
    end
  end

  @spec create_group_post_survey(integer | String.t(), map()) ::
          {:ok, GroupPostSurvey.t()} | {:error, Ecto.Changeset.t()}
  def create_group_post_survey(group_id, attrs) do
    attrs_with_group = Map.put(attrs, :group_id, group_id)

    %GroupPostSurvey{}
    |> GroupPostSurvey.changeset(attrs_with_group)
    |> Repo.insert()
  end

  @spec update_group_post_survey(GroupPostSurvey.t(), map()) ::
          {:ok, GroupPostSurvey.t()} | {:error, Ecto.Changeset.t() | :not_draft}
  def update_group_post_survey(%GroupPostSurvey{status: "draft"} = survey, attrs) do
    survey
    |> GroupPostSurvey.update_changeset(attrs)
    |> Repo.update()
  end

  def update_group_post_survey(%GroupPostSurvey{}, _attrs), do: {:error, :not_draft}

  @spec update_group_post_survey_status(GroupPostSurvey.t(), String.t()) ::
          {:ok, GroupPostSurvey.t()} | {:error, Ecto.Changeset.t()}
  def update_group_post_survey_status(%GroupPostSurvey{} = survey, status) do
    survey
    |> GroupPostSurvey.status_changeset(status)
    |> Repo.update()
  end
end
